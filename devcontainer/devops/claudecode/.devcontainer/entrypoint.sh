...
check_dependencies() {
    ...
    if command -v helm &>/dev/null; then
        log_info "✓ Helm: $(helm version --short 2>/dev/null || helm version | head -1)"
    else
        log_warn "⚠ Helm 未安装（可选）"
    fi
    ...
    if command -v shellcheck &>/dev/null; then
        log_info "✓ shellcheck: $(shellcheck --version | grep -m1 version)"
    else
        log_warn "⚠ shellcheck 未安装（可选）"
    fi
